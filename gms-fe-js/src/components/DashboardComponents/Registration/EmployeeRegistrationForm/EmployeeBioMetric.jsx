...
const EmployeeBioMetric = ({ onNext, onPrevious, onComplete, onSave, initialData = {} }) => {
    ...
    const handleContinue = () => {
        ...
        // Always create a biometric object, even if empty
        const formattedData = hasAnyBiometricData 
            ? formatBiometricData(uploadedFiles, false)
            : { biometric: undefined };

        console.log('Final biometric formattedData:', formattedData);
        console.log('=== END DEBUG ===');

        // Save the data (if any) and continue
        if (hasAnyBiometricData && onSave) {
            onSave('bio-metric', formattedData);
        }
        if (onComplete) {
            onComplete();
        }
        ...
        <div className="flex-1 bg-white p-8">
            ...
            <div className="mb-8">
                <aside className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-gray-900">Bio-Metric (Optional)</h2>
            ...
            <div className="flex justify-center space-x-4 pt-8 mt-8">
                ...
                >
                    {Object.values(uploadedFiles).some(value => value) ? "Complete with Biometric" : "Skip Biometric"}
                </button>
            </div>
        </div>
    );
